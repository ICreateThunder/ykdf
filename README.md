...
ships with SHA-256/512 checksums, a [Sigstore](https://www.sigstore.dev/)
keyless signature bundle (`.sigstore.json`), and SLSA build provenance
(`.intoto.jsonl`). Both the signature and the provenance are produced by the
release workflow's GitHub OIDC identity and logged in the public Rekor
transparency log.
